...
@section('content')
<h1 class="text-2xl font-bold">Modification du compte</h1>
@if(session('success'))
...
<table class="table-auto w-full mt-4">
    ...
    <tbody>
        @if ($jobworker->email === auth()->user()->email)
        <tr>
            <th scope="row" class="border px-4 py-2">{{ $jobworker->id }}</th>
            <td class="border px-4 py-2">
                <a class="bg-blue-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" href="/inscription/delete/{{$jobworker->id }}" onclick="event.preventDefault();
            ...
            <td class="border px-4 py-2">
                <a class="bg-blue-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" href="/inscription/edit/{{$jobworker->id }}">
                    Modifier
                </a>
            </td>
        </tr>
